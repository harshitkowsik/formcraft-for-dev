...
export function PreviewTab({ config, theme = 'dark' }: PreviewTabProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    let emailBody = `New Form Submission (${config.formTitle}):\n\n`;
    config.fields
      .filter((f) => f.enabled)
      .forEach((f) => {
    ...
    emailBody += `\n--- Generated by FormCraft Generator ---`;

    const subject = encodeURIComponent(config.emailSubjectPrefix || 'New Form Response');
      ...
      <div className={`${cardClass} border ${config.borderRadius} p-8 relative`}>
        {submittedData ? (
          <div className="text-center py-6 space-y-4">
            ...
            <button
              onClick={handleReset}
              className={`inline-flex items-center gap-2 px-4 py-2 ${isDark ? 'bg-white hover:bg-zinc-200 text-black' : 'bg-black hover:bg-zinc-800 text-white'} text-xs font-semibold rounded transition-all`}
            >
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-2 flex items-center gap-2">
                <button
                  type="submit"
                  className={`flex-1 py-2.5 px-4 ${isDark ? 'bg-white hover:bg-zinc-200 text-black' : 'bg-black hover:bg-zinc-800 text-white'} text-xs font-semibold ${config.borderRadius} transition-all flex items-center justify-center gap-1.5`}
                >
                ...
                {config.showResetButton && (
                  <button
                    type="button"
                    onClick={handleReset}
                    className={`py-2.5 px-4 ${isDark ? 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border-zinc-700' : 'bg-zinc-100 hover:bg-zinc-200 text-zinc-700 border-zinc-300'} text-xs font-medium ${config.borderRadius} transition-all border`}
                  >
                    Reset
                  </button>
                )}
